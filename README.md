...
Pipeline Exquis is a code experiment carried out by Juliet van Rosendaal (KU Leuven), Megan Leal Causton (Vrije Universiteit Brussel), and Hina Vuijk. It was first conducted in May 2024 in preparation of the conference [Code as Conversation: Transmedia Dialogues Around Critical Code Studies](https://www.cdh.cam.ac.uk/events/37778/) on 1 June 2024 in Cambridge, UK which is situated in the field of [critical code studies](#glossary).
...
Note: the landscape of ML development, including the increasing standardisation and automation of pipeline stages forms a key backdrop to this experiment. The pipeline exquis sits at the intersection of these trends and the surrealist logic of isolated, sequential contribution. See the published article for the full theoretical framing.

## 1. From Cadavre Exquis to Pipeline Exquis

In this section, the process of creating a Cadavre Exquis is first described by steps, then adapted to match the process of creating an ML pipeline. 
...
1. Designer1, Designer2, Designer3 and a rectangular piece of paper is collected
2. Designer1:
	1. draws **head** on +/- **upper** 1/3 of paper
	2. folds **head** away so that it can't be seen by Designer2 and Designer3
...
3. Designer2:
	...
	2. folds **upper body** away so that it can't be seen by Designer1 and Designer3
...
2. Coder1
	...
	2. writes preprocessed data to **preprocessed_data.csv** so that it can't be seen by Coder2 and Coder3
	3. writes down information in division_of_labour.md (the "meta file") about preprocessed_data.csv (functioning as "tick marks") with which Coder2 will continue the data modelling. Coder2 and Coder3 do not view Coder1's notebook before completing their own stage.
3. Coder2
	1. creates **data_modelling.py**
	2. writes model to **model.csv** so that it can't be seen by Coder1 and Coder3
	3. writes down minimal technical information in division_of_labour.md about model.csv with which Coder3 will continue the model deployment. 
4. Coder3
	1. creates **model_deployment.py**
5. All the files are opened and/or the deployment file is run, and Coder1, Coder2 and Coder3 collectively examine their work.
...
The following questions and notes are working materials from the exercise itself. The discussion notes are from the pipeline reveal session held on 20 May 2024.
...
-	For Coder2
	...
	- Ended up using variables that Coder1 considered not very useful
-	For Coder3
	- “Death calculator”
...
## Bibliography
See the article for full references - https://link.springer.com/article/10.1007/s00146-026-02963-5
